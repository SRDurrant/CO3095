...
def test_view_school_rankings(capsys):
...
def test_view_school_rankings_multiple_levels(capsys):

    SCHOOLS.clear()
    RATINGS.clear()

    SCHOOLS.extend([
        {"school_id": 1, "name": "A", "level": "primary"},
        {"school_id": 2, "name": "B", "level": "secondary"},
    ])
    ...
    assert "Primary Schools Ranking" in out
    assert "Secondary Schools Ranking" in out