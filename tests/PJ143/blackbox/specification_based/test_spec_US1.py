...
def test_add_primary_school_success():
    ...
    SCHOOLS.clear()
    inputs = ["Primary School", "1", "Derby", "2"]
    ...
    assert SCHOOLS[0]["location"] == "Derby"


def test_add_secondary_school():
    """Adding a secondary level school"""
    SCHOOLS.clear()
...
def test_add_combined_school():
    """Adding a combined school successfully"""
    SCHOOLS.clear()
    inputs = iter(["Combined Academy", "3", "Derby", "2"])
...
def test_invalid_level_choice():
    """Reject invalid level choice"""
    SCHOOLS.clear()
    inputs = iter(["Valid School Name", "4", "0"])
    output = []
...
def test_add_multiple_schools():
    """Adding multiple schools in one session"""
    SCHOOLS.clear()
    inputs = iter([
